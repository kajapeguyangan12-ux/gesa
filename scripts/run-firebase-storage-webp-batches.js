...
const DEFAULT_PROGRESS_FILE = "firebase-storage-webp-progress.json";
const CONVERTER_SCRIPT = path.resolve(process.cwd(), "scripts/convert-firebase-storage-images-to-webp.js");
...
function buildSummaryFilePath(batchOffset) {
  return path.resolve(
    process.cwd(),
    `firebase-storage-webp-summary-${Date.now()}-${batchOffset}.json`
  );
}
...
function runBatch(options, offset) {
  ...
  console.log(
    `[batch] offset=${offset} batchSize=${options.batchSize} concurrency=${options.concurrency} dryRun=${options.dryRun}`
  );
...
function createInitialProgress(options, startOffset) {
  return {
    kind: "firebase-storage-webp-progress",
...
function main() {
  ...
  while (true) {
    if (options.maxBatches && completedThisRun >= options.maxBatches) {
      break;
    }
    ...
    if (summary.selectedCount === 0 || summary.selectedCount < options.batchSize) {
      break;
    }
