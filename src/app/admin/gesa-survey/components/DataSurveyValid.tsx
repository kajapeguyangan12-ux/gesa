...
export default function DataSurveyValid() {
  ...
  const fetchStatistics = async () => {
    try {
      ...
      const qExisting = query(existingRef, where("status", "==", "tervalidasi"));
      const qPropose = query(proposeRef, where("status", "==", "tervalidasi"));
  ...
  if (selectedCategory === "existing") {
    return <SurveyExistingDetail onBack={() => setSelectedCategory(null)} />;
  }

  if (selectedCategory === "propose") {
    return <SurveyProposeDetail onBack={() => setSelectedCategory(null)} />;
  }
