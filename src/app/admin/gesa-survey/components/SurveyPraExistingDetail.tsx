...
export default function SurveyPraExistingDetail({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/surveys/pra-existing/${encodeURIComponent(id)}`, {
        method: "DELETE",
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(payload?.error || "Gagal menghapus data survey pra-existing.");
      }
  ...
  const handleRestore = async (survey: Survey) => {
    ...
    try {
      const response = await fetch(`/api/admin/surveys/pra-existing/${encodeURIComponent(survey.id)}`, {
        method: "PATCH",
      ...
      if (!response.ok) {
        if (await handleConflictAndReload(response, () => fetchSurveys(), "Gagal memulihkan status survey pra-existing.")) {
